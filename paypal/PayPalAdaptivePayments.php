...
class PayPalAdaptivePaymentsGateway {
	...
	public function preapproval( $payment_id, $amount, $reference_token, $starting_date=null, $ending_date=null ) {
		...
		$create_packet = array(
			...
			'ipnNotificationUrl'           => trailingslashit( home_url() ) . '?ipn=epap&payment_id=' . $payment_id,
			'requestEnvelope'              => $this->envelope,
			'startingDate'                 => isset( $starting_date ) ? $starting_date : apply_filters( 'epap_preapproval_start_date', date( 'c', time() ) ),
			'endingDate'                   => isset( $ending_date ) ? $ending_date : apply_filters( 'epap_preapproval_end_date', date( 'c', time() + 364*86400 ) ),
